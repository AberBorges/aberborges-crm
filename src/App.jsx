...
// ─── AGENDA ─────────────────────────────────────────────
const AGENDA_KEY = "aberborges_agenda_v1";
...
export default function App() {
  ...
  useEffect(()=>{
    try { const s=localStorage.getItem(AGENDA_KEY); if(s) setAgenda(JSON.parse(s)); } catch {}
  },[]);
    ...
    return (
      <div style={{...S.card, borderColor: muyUrgente?"#ef4444":pasada?"#1e2535":BORDER, opacity:pasada?0.6:1}}>
        ...
        <div style={{display:"flex",gap:8,marginTop:4}}>
          ...
          {c.telefono && <button style={{...S.waBtn,flex:1,padding:"7px 0",fontSize:12}} onClick={()=>window.open(`https://wa.me/549${c.telefono.replace(/\D/g,"")}?text=${encodeURIComponent(`Hola ${c.nombre}, te recuerdo que tengo una visita programada para el ${fmtFecha(v.fecha)} a las ${v.hora}. Saludos, Aberturas Borges.`)}`)}>💬 WhatsApp</button>}
