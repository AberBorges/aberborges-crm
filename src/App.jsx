...
export default function App() {
  ...
  const cargarDatos = async () => {
    ...
    try {
      ...
      if (cls && cls.length > 0) {
        const lista = cls.map(r => r.data).filter(Boolean);
        setClientes(lista);
        lsSet(LS_CLIENTES, lista);
      } else if (localCls && localCls.length > 0) {
        // Supabase vacío pero hay datos locales — subirlos
        await supabase.from('clientes').upsert(
          localCls.map(c => ({ id: c.id, num_cliente: c.numCliente, data: c })),
          { onConflict: 'id' }
        );
      }

      if (ag && ag.length > 0) {
        const lista = ag.map(r => r.data).filter(Boolean);
        setAgenda(lista);
        lsSet(LS_AGENDA, lista);
      }
