...
// ═══════════════════════════════════════════════════════════
function AgendaPublica({ agenda, onBack }) {
  const ahora   = new Date();
  const hoy     = ahora.toISOString().split("T")[0];
  const manana  = new Date(ahora.getTime()+86400000).toISOString().split("T")[0];

  const visitasHoy = agenda
    .filter(v=>v.fecha===hoy)
    .sort((a,b)=>a.hora.localeCompare(b.hora));

  const visitasManana = agenda
    .filter(v=>v.fecha===manana)
    .sort((a,b)=>a.hora.localeCompare(b.hora));
  ...
  const Burbuja = ({ v, pasada }) => {
    ...
    return (
      <div style={{display:"flex",gap:10,marginBottom:12,alignItems:"flex-end",opacity:pasada?0.5:1}}>
        <div style={{width:38,height:38,background:"#0c2233",borderRadius:"50%",border:`2px solid ${bColor}`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,flexShrink:0}}>
          🕐
        </div>
        <div style={{background:bgColor,border:`1px solid ${border}`,borderRadius:"18px 18px 18px 4px",padding:"10px 14px",flex:1}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
            ...
            {urgente && <span style={{background:"#ef444422",color:"#ef4444",fontSize:10,fontWeight:700,padding:"2px 8px",borderRadius:99,border:"1px solid #ef444444"}}>
              {diffMin<=0?"Ahora":"En "+diffMin+" min"}
            </span>}
          ...
          {v.clienteTelefono && (
            <a href={`tel:${v.clienteTelefono}`} style={{display:"block",color:"#00b4d8",fontSize:13,marginBottom:2,textDecoration:"none"}}>
              📞 {v.clienteTelefono}
            </a>
          )}
          ...
          {v.clienteTelefono && (
            <a href={`https://wa.me/598${v.clienteTelefono.replace(/\D/g,"")}?text=${encodeURIComponent(`Hola ${v.clienteNombre||""}, te confirmo la visita de hoy a las ${v.hora} hs. Saludos, Aberturas Borges.`)}`}
              ...
              style={{display:"inline-flex",alignItems:"center",gap:5,marginTop:8,background:"#0d2b22",color:"#25d366",border:"1px solid #128c7e55",borderRadius:8,padding:"5px 12px",fontSize:12,fontWeight:700,textDecoration:"none"}}>
              💬 WhatsApp
            </a>
          )}
        </div>
  ...
  return (
    <div style={{minHeight:"100vh",background:"#090e18",fontFamily:"'DM Sans','Segoe UI',sans-serif",paddingBottom:40}}>
      <header style={{background:"#0d1626",borderBottom:"1px solid #162035",padding:"12px 16px",display:"flex",alignItems:"center",gap:10,position:"sticky",top:0,zIndex:20}}>
        ...
        <div style={{fontSize:26,color:"#00b4d8"}}>⬡</div>
        <div>
          <div style={{fontWeight:900,fontSize:17,color:"#f1f5f9",letterSpacing:"-0.5px"}}>Agenda</div>
          <div style={{fontSize:11,color:"#475569"}}>Visitas del día</div>
        </div>
      </header>
      ...
      <div style={{padding:"16px 16px 0",maxWidth:500,margin:"0 auto"}}>
        {noHayNada ? (
          <div style={{textAlign:"center",color:"#334155",padding:"60px 0",fontSize:15}}>
            ...
            <span style={{fontSize:13,color:"#1e2a3a"}}>Ingresá a un rol para agregar visitas.</span>
          </div>
        ) : (
          <>
            {visitasHoy.length>0 && (
              <div style={{marginBottom:20}}>
                <div style={{fontSize:12,fontWeight:700,color:"#00b4d8",textTransform:"uppercase",letterSpacing:"0.07em",marginBottom:12}}>
                  📅 Hoy — {fmtFecha(hoy)} ({visitasHoy.length})
                </div>
                {visitasHoy.map(v=><Burbuja key={v.id} v={v} pasada={new Date(v.fecha+"T"+v.hora)<ahora}/>)}
              </div>
            )}
            {visitasManana.length>0 && (
              <div style={{marginBottom:20}}>
                <div style={{fontSize:12,fontWeight:700,color:"#a78bfa",textTransform:"uppercase",letterSpacing:"0.07em",marginBottom:12}}>
                  📌 Mañana — {fmtFecha(manana)} ({visitasManana.length})
                </div>
            ...
            {proximas.length>0 && (
              <div style={{marginBottom:20}}>
                ...
                {proximas.map(v=>(
                  <div key={v.id} style={{background:"#0d1626",border:"1px solid #162035",borderRadius:12,padding:"12px 14px",marginBottom:8}}>
...
function DetailView({cliente,role,agenda,onSaveAgenda,onUpdate,onDelete,showToast}){
  ...
  const TABS=[
    ...
    {key:"agenda", label:"📅 Agenda",   show:true},
