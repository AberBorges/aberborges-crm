...
const TIPOS = [
  ...
  {
    id:"guillotina", label:"Guillotina", icon:"🪟", categoria:"ventana",
    configs:[
      {id:"g1h",label:"1 hoja móvil",  desc:"Hoja inferior desliza hacia arriba, superior fija."},
      {id:"g2h",label:"2 hojas móviles",desc:"Ambas hojas deslizan verticalmente y se solapan."},
    ],
  },
  {
...
function AberturaSVG({ tipoId, configId, w=90, h=70, monoblock=false }) {
  ...
  // ── GUILLOTINA ──────────────────────────────────────────
  // Como corrediza pero vertical: 2 hojas apiladas, riel horizontal al medio, flechas ↕
  if (tipoId === "guillotina") {
    const dosMoviles = configId === "g2h";
    const gx=fx(3), gw=fx(94);
    const midY = fy2(50);
    const topGy=fy2(3), topGh=(midY-topGy)-1;
    const botGy=midY+1, botGh=(fy2(97)-botGy);

    // vertical slide arrows ↕
    const SlideArrowsV = ({cx,cy}) => <>
      <line x1={cx} y1={cy-fh*0.1} x2={cx} y2={cy+fh*0.1} stroke={AR} strokeWidth={1.2}/>
      <polygon points={`${cx},${cy-fh*0.1} ${cx-fw*0.025},${cy-fh*0.06} ${cx+fw*0.025},${cy-fh*0.06}`} fill={AR}/>
      <polygon points={`${cx},${cy+fh*0.1} ${cx-fw*0.025},${cy+fh*0.06} ${cx+fw*0.025},${cy+fh*0.06}`} fill={AR}/>
    </>;

    return <svg width={w} height={h}>
      <Cajon/><Frame/>
      {/* hoja superior — fija en g1h, móvil en g2h */}
      {dosMoviles
        ? <><Glass x={gx} y={topGy} rw={gw} rh={topGh}/><DiagX x={gx} y={topGy} rw={gw} rh={topGh}/></>
        : <rect x={gx} y={topGy} width={gw} height={topGh} fill={GL+"55"} stroke={GS+"55"} strokeWidth={1} strokeDasharray="3,2"/>
      }
      {/* hoja inferior — siempre móvil */}
      <Glass x={gx} y={botGy} rw={gw} rh={botGh}/>
      <DiagX x={gx} y={botGy} rw={gw} rh={botGh}/>
      {/* riel horizontal donde se cruzan las hojas */}
      <line x1={gx-1} y1={midY} x2={gx+gw+1} y2={midY} stroke={GS+"77"} strokeWidth={2}/>
      <SlideArrowsV cx={fw*0.82} cy={midY}/>
    </svg>;
  }
...
export default function App() {
  ...
  ,[clientes,search,filterSt]);
  const counts = useMemo(()=>Object.keys(STATUS_META).reduce((acc,s)=>{acc[s]=clientes.filter(c=>c.status===s).length;return acc;},{})
  ,[clientes]);
  const onSelectCliente = useCallback((c)=>{setSelected(c);setView("detalle");},[]);
...
function AgendaView({ agenda, clientes, onSave, onBack, onVerCliente, onDeleteCliente, showToast }) {
  ...
  // ── Eliminar cliente desde Agenda, con confirmación por PIN ──
  const [delPinFor, setDelPinFor] = useState(null); // id del cliente a eliminar
  const [pinVal, setPinVal] = useState("");
  const [pinErr, setPinErr] = useState(false);
  const confirmarEliminarCliente = () => {
    if (pinVal !== "7777") { setPinErr(true); return; }
    onDeleteCliente(delPinFor);
    setDelPinFor(null); setPinVal(""); setPinErr(false);
  ...
  const VisitaCard = ({ v, pasada }) => {
    ...
    return (
      <div style={{...S.card, borderColor: muyUrgente?"#ef4444":pasada?"#1e2535":BORDER, opacity:pasada?0.6:1}}>
        ...
        {c.id && (
          delPinFor===c.id ? (
            <div style={{marginTop:10,background:"#1a0a0a",border:"1px solid #450a0a",borderRadius:10,padding:12}}>
              <div style={{color:"#fca5a5",fontSize:13,marginBottom:8}}>Ingresá el PIN para eliminar a <b>{c.nombre}</b> definitivamente:</div>
              <input style={{...S.inp,marginBottom:8}} type="password" inputMode="numeric" placeholder="PIN"
                value={pinVal} autoFocus
                onChange={e=>{setPinVal(e.target.value);setPinErr(false);}}
                onKeyDown={e=>{if(e.key==="Enter")confirmarEliminarCliente();}}/>
              {pinErr && <div style={{color:"#f87171",fontSize:12,marginBottom:8}}>PIN incorrecto</div>}
              <div style={{display:"flex",gap:8}}>
                <button style={S.ghostBtn} onClick={()=>{setDelPinFor(null);setPinVal("");setPinErr(false);}}>Cancelar</button>
                <button style={{...S.primaryBtn,background:"#ef4444",flex:1}} onClick={confirmarEliminarCliente}>Confirmar eliminación</button>
              </div>
            </div>
          ) : (
            <button style={{background:"transparent",color:"#7f1d1d",border:"none",fontSize:11,marginTop:6,cursor:"pointer",padding:0}}
              onClick={()=>{setDelPinFor(c.id);setPinVal("");setPinErr(false);}}>🗑️ Eliminar cliente</button>
          )
        )}
      </div>
    );
...
function ObrasActivasView({ clientes, onSelect, onUpdate, onBack, showToast }) {
  ...
  const TIPOS_LABEL = {corrediza:"Ventana Corrediza",proyectante:"Ventana Proyectante",batiente:"Ventana Batiente",guillotina:"Ventana Guillotina",puerta_corrediza:"Puerta Corrediza",puerta_batiente:"Puerta Batiente",fijo:"Paño Fijo",mampara_std:"Mampara Estándar",mampara_f1:"Mampara F1",mampara_l:"Mampara en L",persiana:"Persiana Catalana"};
...
function FabricaListView({ clientes, onSelect, onUpdate, showToast }) {
  const [vista, setVista] = useState("fabricar"); // "fabricar" | "activas" | "entregadas"
  const fmtF = d => { if(!d) return "—"; const [y,m,day]=d.split("-"); return `${day}/${m}/${y}`; };
  const TIPOS_LABEL = {corrediza:"Ventana Corrediza",proyectante:"Ventana Proyectante",batiente:"Ventana Batiente",guillotina:"Ventana Guillotina",puerta_corrediza:"Puerta Corrediza",puerta_batiente:"Puerta Batiente",fijo:"Paño Fijo",mampara_std:"Mampara Estándar",mampara_f1:"Mampara F1",mampara_l:"Mampara en L",persiana:"Persiana Catalana"};
  ...
  const entregadas = clientes.filter(c=>c.status==="entregado"&&(c.aberturas||[]).length>0)
  ...
  return (
    <div style={{padding:"12px 12px 40px",maxWidth:520,margin:"0 auto"}}>
      ...
      <div style={{display:"flex",gap:6,marginBottom:16,background:"#0d1626",border:"1px solid #162035",borderRadius:12,padding:4}}>
        {[["fabricar","🔧 Fabricar"],["activas","📦 Obras activas"],["entregadas","✅ Entregadas"]].map(([k,l])=>(
          <button key={k} style={{flex:1,background:vista===k?ACCENT:"transparent",color:vista===k?"#090e18":"#64748b",border:"none",borderRadius:9,padding:"9px 0",fontWeight:700,fontSize:12,cursor:"pointer"}}
      ...
      {/* ── VISTA OBRAS ENTREGADAS ── */}
      {vista==="entregadas"&&(
        <>
          {entregadas.length===0&&(
            <div style={{textAlign:"center",color:"#334155",padding:"60px 0",fontSize:15}}>
              Todavía no hay obras entregadas.<br/>
              <span style={{fontSize:13,color:"#1e2a3a"}}>Quedarán registradas acá cuando se complete una entrega.</span>
            </div>
          )}
          {entregadas.map(c=>(
            <div key={c.id} style={{marginBottom:10}}>
              <div style={{fontSize:11,color:"#34d399",fontWeight:700,marginBottom:4,paddingLeft:2}}>
                ✅ Completada {c.fechaCompletada?fmtF(c.fechaCompletada):""}
              </div>
              <ObraActivaCard c={c}/>
            </div>
          ))}
...
const ListView = memo(function ListView({role,filtered,counts,clientes,search,setSearch,filterSt,setFilterSt,onSelect,agenda}){
    ...
    <div style={S.page}>
      ...
      <div style={{paddingBottom:40}}>
        {filtered.length===0
          ...
          : filtered.map(c=><ClientCard key={c.id} c={c} role={role} onSelect={onSelect}/>)
        }
      </div>
    </div>
  );
...
const ClientCard = memo(function ClientCard({c,role,onSelect}){
  const sm=STATUS_META[c.status];
  ...
  const nAb=(c.aberturas||[]).reduce((s,a)=>s+(parseInt(a.cantidad)||0),0);
  return (
    <div style={S.clientCard} onClick={()=>onSelect(c)}>
  ...
  return (
    <div style={S.page}>
      ...
      <div style={S.detHead}>
        ...
        <button style={S.waBtn} onClick={()=>window.open(`https://wa.me/598${c.telefono.replace(/\D/g,"")}${c.presupuesto?`?text=${buildWA(c)}`:""}`,"_blank")}>
