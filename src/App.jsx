    ...
    <div style={S.app}>
      ...
      <header style={S.header} className="no-print">
        <div style={S.hLeft}>
          ...
          <img src={LOGO_B64} style={{...S.headerLogo,width:30,height:30}} alt="AB"/>
          <div>
            <div style={{...S.headerBrand,fontSize:13}}>{BRAND}</div>
            <div style={{...S.headerRole,color:roleMeta.color}}>{roleMeta.icon} {roleMeta.label}</div>
          </div>
        </div>
        <div style={{display:"flex",flexDirection:"column",gap:4,alignItems:"flex-end",flexShrink:0}}>
          {/* Fila 1: Agenda, Nuevo, Salir */}
          <div style={{display:"flex",gap:5}}>
            {view==="lista"&&(
              <button style={{...S.ghostBtn,fontSize:12,padding:"6px 10px"}}
                onClick={()=>setView("agenda")}>📅 Agenda</button>
            )}
            {view==="lista"&&(role==="vendedor"||role==="admin"||role==="presupuestador")&&(
              <button style={{...S.primaryBtn,background:ACCENT,color:BG,padding:"6px 12px",fontSize:12}} onClick={()=>{
            ...
            <button style={{...S.ghostBtn,fontSize:12,padding:"6px 10px"}} onClick={()=>{setRole(null);setPinRole(null);}}>Salir</button>
          </div>
          {/* Fila 2: Obras, Deudores */}
          {view==="lista"&&(
            <div style={{display:"flex",gap:5}}>
              <button style={{...S.ghostBtn,fontSize:12,padding:"5px 10px",borderColor:"#34d39955",color:"#34d399"}}
                onClick={()=>setView("obras")}>📦 Obras</button>
              {(role==="presupuestador"||role==="admin"||role==="deudores")&&(
                <button style={{...S.ghostBtn,fontSize:12,padding:"5px 10px",borderColor:"#f43f5e55",color:"#f43f5e"}}
