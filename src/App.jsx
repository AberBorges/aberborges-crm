...
function AgendaView({ agenda, clientes, onSave, onBack, onVerCliente, showToast, onUpdateCliente }) {
  ...
  const VisitaCard = ({ v, pasada }) => {
    ...
    return (
      <div style={{...S.card, borderColor: muyUrgente?"#ef4444":pasada?"#1e2535":BORDER, opacity:pasada?0.7:1}}>
        {muyUrgente && (
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
          <div>
            <div style={{fontWeight:800,fontSize:16,color:"#f1f5f9"}}>{c.nombre||v.clienteNombre||"Cliente"}</div>
        ...
        <div style={{display:"flex",gap:"12px 20px",flexWrap:"wrap",fontSize:13,color:"#94a3b8",marginBottom:8}}>
          {(c.telefono||v.clienteTelefono) && <span>📞 <b style={{color:"#f1f5f9"}}>{c.telefono||v.clienteTelefono}</b></span>}
        ...
        <div style={{display:"flex",gap:6,marginTop:4,flexWrap:"wrap"}}>
          ...
          <button style={{background:"#0a1a2a",border:"1px solid #1e40af",color:"#60a5fa",borderRadius:8,padding:"6px 10px",fontSize:12,fontWeight:600,cursor:"pointer"}}
            onClick={()=>abrirEditarVisita(v)}>✏️ Editar</button>
          {c.id && <button style={{background:"#0d2010",border:"1px solid #166534",color:"#4ade80",borderRadius:8,padding:"6px 10px",fontSize:12,fontWeight:600,cursor:"pointer"}}
            onClick={()=>abrirEditarCliente(c)}>👤 Datos</button>}
          <button style={{...S.ghostBtn,fontSize:12,padding:"6px 10px",color:"#f87171",borderColor:"#450a0a"}} onClick={()=>eliminar(v.id)}>✕</button>
        </div>
      </div>
    );
  };
