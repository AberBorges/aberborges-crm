...
function AberturaSVG({ tipoId, configId, w=90, h=70, monoblock=false }) {
  ...
  const frameC = "#2a4a6e";
  const glassC = ACCENT+"28";
  const glassS = ACCENT;
  const alumC  = "#1a2e48";
  const alumS  = "#2a4060";
  const diagC  = ACCENT+"66";
  const arrowC = ACCENT;
  const monoC  = "#fb923c";

  const DiagX = ({x,y,rw,rh}) => <>
    <line x1={x} y1={y} x2={x+rw} y2={y+rh} stroke={diagC} strokeWidth={1.2}/>
    <line x1={x+rw} y1={y} x2={x} y2={y+rh} stroke={diagC} strokeWidth={1.2}/>
  </>;

  const SlideArrow = ({cx,cy}) => <>
    <line x1={cx-fw*0.12} y1={cy} x2={cx+fw*0.12} y2={cy} stroke={arrowC} strokeWidth={1.3}/>
    <polygon points={`${cx-fw*0.12},${cy} ${cx-fw*0.08},${cy-fh*0.03} ${cx-fw*0.08},${cy+fh*0.03}`} fill={arrowC}/>
    <polygon points={`${cx+fw*0.12},${cy} ${cx+fw*0.08},${cy-fh*0.03} ${cx+fw*0.08},${cy+fh*0.03}`} fill={arrowC}/>
  </>;

  const Cajon = () => monoblock ? <>
    <rect x={0} y={0} width={fw} height={cajH} rx={2} fill={alumC} stroke={monoC} strokeWidth={1.5}/>
    {[0.3,0.6,0.85].map((f,i)=>(
      <line key={i} x1={6} y1={cajH*f} x2={fw-6} y2={cajH*f} stroke={monoC+"55"} strokeWidth={0.9}/>
    ))}
    <rect x={fw*0.35} y={1.5} width={fw*0.3} height={cajH*0.32} rx={1.5} fill={monoC} opacity={0.55}/>
  </> : null;

  const Frame = () => <rect x={0} y={fy} width={fw} height={fh} rx={2} fill={alumC} stroke={frameC} strokeWidth={2.5}/>;

  // ── CORREDIZA ──
  if (tipoId==="corrediza") {
    const Panel = ({x,pw,fixed}) => (
      <rect x={fx(x)} y={fy2(3)} width={fx(pw)} height={fh*0.92}
        fill={fixed?glassC+"44":glassC} stroke={fixed?glassS+"44":glassS}
        strokeWidth={fixed?1:1.8} strokeDasharray={fixed?"3,2":"none"}/>
    );
    let panels=[];
    if (configId==="c2h2r") panels=[<Panel key={0} x={2} pw={47}/>,<Panel key={1} x={51} pw={47}/>];
    else if (configId==="c3h3r") panels=[<Panel key={0} x={2} pw={30}/>,<Panel key={1} x={34} pw={30}/>,<Panel key={2} x={66} pw={30}/>];
    else if (configId==="c3h2r") panels=[<Panel key={0} x={2} pw={30}/>,<Panel key={1} x={34} pw={30} fixed/>,<Panel key={2} x={66} pw={30}/>];
    else if (configId==="c4h2r") panels=[0,24,50,74].map((x,i)=><Panel key={i} x={x+1} pw={22}/>);
    else panels=[0,24,50,74].map((x,i)=><Panel key={i} x={x+1} pw={22}/>);
    return <svg width={w} height={h} style={{overflow:"visible"}}>
      <Cajon/><Frame/>{panels}
      <line x1={0} y1={fy2(96)} x2={fw} y2={fy2(96)} stroke={glassS+"55"} strokeWidth={2}/>
      <SlideArrow cx={fw/2} cy={fy2(55)}/>
    </svg>;
  }
  ...
  if (tipoId==="proyectante") {
    ...
    return <svg width={w} height={h}>
      <Cajon/><Frame/>
      {two?[0,1].map(i=>{
        const gx=fx(i===0?2:52),gw=fx(46),gy=fy2(3),gh=fh*0.92;
        return <g key={i}>
          <rect x={gx} y={gy} width={gw} height={gh} fill={glassC} stroke={glassS} strokeWidth={1.5}/>
          <DiagX x={gx} y={gy} rw={gw} rh={gh}/>
          <line x1={gx} y1={gy} x2={gx+gw} y2={gy} stroke={arrowC} strokeWidth={2.2}/>
          <line x1={gx+gw/2} y1={fy2(68)} x2={gx+gw/2} y2={fy2(28)} stroke={arrowC} strokeWidth={1.2}/>
          <polygon points={`${gx+gw/2},${fy2(24)} ${gx+gw/2-fw*0.04},${fy2(34)} ${gx+gw/2+fw*0.04},${fy2(34)}`} fill={arrowC}/>
        </g>;
      }):<>
        <rect x={fx(3)} y={fy2(3)} width={fx(94)} height={fh*0.92} fill={glassC} stroke={glassS} strokeWidth={1.5}/>
        <DiagX x={fx(3)} y={fy2(3)} rw={fx(94)} rh={fh*0.92}/>
        <line x1={fx(3)} y1={fy2(3)} x2={fx(97)} y2={fy2(3)} stroke={arrowC} strokeWidth={2.5}/>
        <line x1={fw/2} y1={fy2(68)} x2={fw/2} y2={fy2(26)} stroke={arrowC} strokeWidth={1.3}/>
        <polygon points={`${fw/2},${fy2(22)} ${fw/2-fw*0.05},${fy2(33)} ${fw/2+fw*0.05},${fy2(33)}`} fill={arrowC}/>
      </>}
    </svg>;
  }

  // ── BATIENTE VENTANA — vista desde ADENTRO, solo interior ──
  if (tipoId==="batiente") {
    const doble=configId==="b2h";
    // b1hd = hoja derecha (bisagra izquierda)
    // b1hi = hoja izquierda (bisagra derecha)
    const bisIzq=configId==="b1hd";
    const Hoja=({gx,gw,gy,gh,bisLeft})=>{
      const arcPath=bisLeft
        ?`M${gx+gw},${gy+gh*0.9} A${gw*0.95},${gh*0.9} 0 0,1 ${gx},${gy+gh*0.45}`
        :`M${gx},${gy+gh*0.9} A${gw*0.95},${gh*0.9} 0 0,0 ${gx+gw},${gy+gh*0.45}`;
      const pivX=bisLeft?gx:gx+gw;
      return <>
        <rect x={gx} y={gy} width={gw} height={gh} fill={glassC} stroke={glassS} strokeWidth={1.8}/>
        <DiagX x={gx} y={gy} rw={gw} rh={gh}/>
        <path d={arcPath} stroke={arrowC+"88"} strokeWidth={1.2} strokeDasharray="4,3" fill={arrowC+"0a"}/>
        <rect x={pivX-2} y={gy+gh*0.2} width={4} height={gh*0.1} rx={1} fill={alumS}/>
        <rect x={pivX-2} y={gy+gh*0.7} width={4} height={gh*0.1} rx={1} fill={alumS}/>
        <rect x={bisLeft?gx+gw-8:gx+2} y={gy+gh*0.44} width={6} height={gh*0.12} rx={2} fill={glassS+"cc"}/>
      </>;
    };
    return <svg width={w} height={h}>
      <Cajon/><Frame/>
      {doble
        ?<><Hoja gx={fx(2)} gw={fx(46)} gy={fy2(3)} gh={fh*0.92} bisLeft/><Hoja gx={fx(52)} gw={fx(46)} gy={fy2(3)} gh={fh*0.92} bisLeft={false}/></>
        :<Hoja gx={fx(3)} gw={fx(94)} gy={fy2(3)} gh={fh*0.92} bisLeft={bisIzq}/>
      }
      <text x={fw/2} y={h-1} textAnchor="middle" fontSize={6.5} fill={arrowC+"99"} fontWeight="bold">
  ...
  if (tipoId==="puerta_corrediza") {
    ...
    return <svg width={w} height={h}>
      <Cajon/>
      <rect x={0} y={fy} width={fw} height={fh} rx={2} fill={alumC} stroke={frameC} strokeWidth={2.5}/>
      {one
        ?<rect x={fx(3)} y={fy2(2)} width={fx(94)} height={fh*0.96} fill={glassC} stroke={glassS} strokeWidth={1.8}/>
        :<><rect x={fx(2)} y={fy2(2)} width={fx(46)} height={fh*0.96} fill={glassC} stroke={glassS} strokeWidth={1.8}/>
           <rect x={fx(52)} y={fy2(2)} width={fx(46)} height={fh*0.96} fill={glassC+"55"} stroke={glassS+"55"} strokeWidth={1}/></>
      }
      <line x1={0} y1={fy2(97)} x2={fw} y2={fy2(97)} stroke={glassS+"55"} strokeWidth={2.5}/>
      <SlideArrow cx={fw/2} cy={fy2(50)}/>
    </svg>;
  }

  // ── PUERTA BATIENTE — vista siempre desde AFUERA ──
  // INTERIOR (empuja): hoja se aleja → arco punteado azul que sube
  //   Interior Izquierda (pb1hi_int): hoja va a IZQ → bisagra DERECHA → bisLeft=false
  //   Interior Derecha   (pb1hd_int): hoja va a DER → bisagra IZQUIERDA → bisLeft=true
  // EXTERIOR (tira): hoja viene hacia vos → arco sólido naranja que baja
  //   Exterior Izquierda (pb1hi_ext): hoja queda IZQ → bisagra IZQUIERDA → bisLeft=true
  //   Exterior Derecha   (pb1hd_ext): hoja queda DER → bisagra DERECHA → bisLeft=false
  if (tipoId==="puerta_batiente") {
    ...
    const Hoja=({gx,gw,gy,gh,bL,exterior})=>{
      // Interior: arco punteado que va hacia arriba (puerta empujada lejos)
      // Exterior: arco sólido que va hacia abajo (puerta jalada hacia vos)
      ...
      return <>
        ...
        <DiagX x={gx} y={gy} rw={gw} rh={gh}/>
        <path d={arcPath}
          stroke={exterior?"#fb923c":arrowC+"77"}
          strokeWidth={exterior?1.8:1.2}
          strokeDasharray={exterior?"none":"4,3"}
          fill={exterior?"#fb923c0d":"none"}/>
        {/* bisagras */}
        <rect x={pivX-2.5} y={gy+gh*0.18} width={5} height={gh*0.1} rx={1} fill={alumS}/>
        <rect x={pivX-2.5} y={gy+gh*0.72} width={5} height={gh*0.1} rx={1} fill={alumS}/>
        {/* manija */}
        <rect x={bL?gx+gw-9:gx+3} y={gy+gh*0.43} width={7} height={gh*0.14} rx={2.5} fill={glassS+"cc"}/>
      </>;
    };
    ...
    return <svg width={w} height={h}>
      ...
      <rect x={0} y={fy} width={fw} height={fh} rx={2} fill={alumC} stroke={frameC} strokeWidth={2.5}/>
      {doble
        ?<><Hoja gx={fx(2)} gw={fx(46)} gy={fy2(2)} gh={fh*0.96} bL exterior={ext}/><Hoja gx={fx(52)} gw={fx(46)} gy={fy2(2)} gh={fh*0.96} bL={false} exterior={ext}/></>
        :<Hoja gx={fx(2)} gw={fx(96)} gy={fy2(2)} gh={fh*0.96} bL={bisLeft} exterior={ext}/>
      }
      <text x={fw/2} y={h-1} textAnchor="middle" fontSize={6.5}
        fill={ext?"#fb923c99":arrowC+"99"} fontWeight="bold">{lbl}</text>
    </svg>;
  }
  ...
  if (tipoId==="persiana") {
    return <svg width={w} height={h}>
      <rect x={0} y={0} width={w} height={h*0.22} rx={2} fill={alumC} stroke={"#fb923c"} strokeWidth={1.5}/>
      {[0.3,0.6,0.85].map((f,i)=>(
        <line key={i} x1={6} y1={h*0.22*f} x2={w-6} y2={h*0.22*f} stroke={"#fb923c55"} strokeWidth={0.9}/>
      ))}
      <rect x={w*0.35} y={2} width={w*0.3} height={h*0.22*0.35} rx={1.5} fill={"#fb923c"} opacity={0.55}/>
      <rect x={0} y={h*0.22} width={w} height={h*0.78} rx={2} fill={alumC} stroke={frameC} strokeWidth={2}/>
      {Array.from({length:7},(_,i)=>(
        <rect key={i} x={fx(5)} y={h*0.22+(i*(h*0.78*0.9))/7+h*0.02}
          width={fx(90)} height={h*0.78*0.1} rx={1}
          fill={i%2===0?"#fb923c30":"#fb923c18"} stroke={"#fb923c55"} strokeWidth={0.8}/>
      ))}
      <text x={fw/2} y={h-2} textAnchor="middle" fontSize={6.5} fill={"#fb923c88"}>
  ...
  if (tipoId==="fijo") {
    return <svg width={w} height={h}>
      <Cajon/><Frame/>
      <rect x={fx(4)} y={fy2(4)} width={fx(92)} height={fh*0.90} fill={glassC} stroke={glassS} strokeWidth={1.5}/>
      <DiagX x={fx(4)} y={fy2(4)} rw={fx(92)} rh={fh*0.90}/>
    </svg>;
  }
  ...
  return <svg width={w} height={h}>
    <Frame/>
    <rect x={fx(4)} y={fy2(4)} width={fx(92)} height={fh*0.90} fill={glassC+"44"} stroke={glassS+"44"} strokeWidth={1}/>
    {Array.from({length:6},(_,i)=>(
      <rect key={i} x={fx(4)} y={fy2(4)+(i*fh*0.90)/6+1}
        width={fx(92)} height={fh*0.90/6-2}
        fill={i%2===0?glassC:glassC+"55"} stroke={glassS+"44"} strokeWidth={0.8}/>
    ))}
  </svg>;
}
...
function buildWA(c) {
  ...
  if (c.obsModo==="describir" && (c.aberturas||[]).length>0) {
    ...
    c.aberturas.forEach(a=>{
      const precio=parseFloat((c.preciosAb||{})[a.id])||0;
      const cant=parseInt(a.cantidad)||1;
      const subtotal=precio*cant;
      const linea="• "+aberturaResumen(a);
      if (subtotal>0) {
        lineas.push(linea);
        lineas.push(`   💵 ${fmtMoney(precio)} × ${cant} = *${fmtMoney(subtotal)}*`);
      } else {
        lineas.push(linea);
      }
    });
      ...
      <div style={{...S.center, justifyContent:"flex-start", paddingTop:30, overflowY:"auto", minHeight:"100vh"}}>
        ...
        <div style={S.roleList}>
          ...
          <button style={{...S.roleRow, borderColor:`${ACCENT}55`, background:`${ACCENT}10`}}
            ...
            <span style={{...S.roleRowLabel, color:ACCENT}}>Agenda del día</span>
            <span style={{color:ACCENT, fontSize:20, marginLeft:"auto"}}>›</span>
          </button>
...
function AgendaPublica({ agenda, clientes, onSave, onBack, onAddCliente }) {
  ...
  const noHayNada=visitasHoy.length===0&&visitasManana.length===0&&proximas.length===0;
...
function DetailView({cliente,role,agenda,onSaveAgenda,onUpdate,onDelete,showToast}){
  ...
  const TABS=[
    ...
    {key:"fab",    label:"🏗️ Fabricar",  show:role==="fabrica"||role==="admin"},
    {key:"obra",   label:"Obra",         show:canM||role==="presupuestador"},
        ...
        <div>
          ...
          <div style={S.card}>
            ...
            <div style={{display:"flex",gap:10,marginBottom:16}}>
              {[
                {s:"aprobado",     label:"⏳ Pendiente",    color:"#34d399"},
                {s:"en_produccion",label:"🔧 En proceso",   color:"#fb923c"},
                {s:"entregado",    label:"📦 Terminada",    color:"#94a3b8"},
              ].map(opt=>(
                <button key={opt.s}
                  style={{flex:1,border:`2px solid ${c.status===opt.s?opt.color:"#1e2535"}`,borderRadius:10,padding:"10px 6px",fontWeight:700,fontSize:12,cursor:"pointer",
            ...
            {c.fechaEntrega&&<div style={{fontSize:13,color:"#64748b"}}>📅 Entrega: <b style={{color:"#f1f5f9"}}>{c.fechaEntrega.split("-").reverse().join("/")}</b></div>}
          </div>
        ...
        <div>
          ...
          {c.obsModo==="describir" && (c.aberturas||[]).length>0 && (
            <div style={S.card}>
              ...
              {(c.aberturas||[]).map((a,i)=>{
                const t=TIPOS.find(t=>t.id===a.tipo);
                const precio=parseFloat((c.preciosAb||{})[a.id])||0;
                const cant=parseInt(a.cantidad)||1;
                const subtotal=precio*cant;
                return (
                  <div key={a.id} style={{borderBottom:`1px solid #0c1525`,padding:"10px 0"}}>
                    <div style={{fontSize:13,color:"#94a3b8",marginBottom:6,lineHeight:1.4}}>{aberturaResumen(a)}</div>
                      ...
                      <div style={{flex:1,minWidth:120}}>
                        ...
                        <input type="number" style={{width:"100%",background:"#090e18",border:"1px solid #1a2845",borderRadius:8,padding:"8px 10px",color:"#e2e8f0",fontSize:14,outline:"none"}}
                          ...
                          onChange={e=>{
                            ...
                            const total=Object.entries(nuevos).reduce((s,[id,p])=>{
                              const ab=(c.aberturas||[]).find(a=>a.id===id);
                              return s+(parseFloat(p)||0)*(parseInt(ab?.cantidad)||1);
                            },0);
                            setC(prev=>({...prev,preciosAb:nuevos,monto:String(total)}));
                          }}/>
                      </div>
                      <div style={{fontSize:12,color:"#64748b"}}>× {cant}</div>
                      <div style={{minWidth:80,textAlign:"right"}}>
                        <div style={{fontSize:10,color:"#475569",marginBottom:2}}>Subtotal</div>
                        <div style={{fontWeight:700,color:subtotal>0?"#34d399":"#334155",fontSize:14}}>{subtotal>0?fmtMoney(subtotal):"—"}</div>
                      </div>
                    </div>
                  </div>
                );
              ...
              <button style={{...S.primaryBtn,width:"100%",marginTop:12}} onClick={()=>save({presupuesto:c.presupuesto,monto:c.monto,preciosAb:c.preciosAb})}>Guardar precios ✓</button>
            </div>
          )}
        ...
        <div style={{flex:1}}>
          ...
          {series.length>0&&(
            <div style={{display:"flex",gap:8,marginBottom:8,flexWrap:"wrap",alignItems:"center"}}>
              ...
              {!["persiana","fijo","celosia"].includes(ab.tipo)&&ab.tipo&&(
                <label style={{display:"flex",alignItems:"center",gap:6,cursor:"pointer",fontSize:12,
