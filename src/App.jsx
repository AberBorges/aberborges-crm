...
function buildWA(c) {
  ...
  if (c.obsModo==="describir" && (c.aberturas||[]).length>0) {
    ...
    c.aberturas.forEach((a,i)=>{
      ...
      // Colocación
      lineas.push(conColoc
        ? `   🔧 _Incluye colocación_`
        : `   ⚙️ _Solo fabricación (sin colocación)_`);
  ...
  lineas.push("");
  lineas.push(`Desde ya agradecemos nos hayan tenido en cuenta.`);
  lineas.push(`🏠 _Aberturas Borges — Hacemos las Aberturas de Tu Hogar_`);
  lineas.push(`📞 _098 056 475 / 4532 5632_`);
...
async function generarWordPresupuesto(c) {
  ...
  // Logo base64 → ArrayBuffer
  const logoB64 = LOGO_B64.replace('data:image/jpeg;base64,','');
    ...
    sections:[{ properties:{page:{size:{width:11906,height:16838},margin:{top:900,right:1100,bottom:900,left:1100}}},
      children:[
        ...
        new Paragraph({ spacing:{before:0,after:100}, children:[
          new ImageRun({ data:logoBytes.buffer, type:"jpg", transformation:{width:100,height:100} }),
...
function VisitaCard({ v, pasada, clientes, ahora, onVerCliente, onEditar, onEditarCliente, onEliminar }) {
  ...
  return (
    <div style={{...S.card, borderColor: muyUrgente?"#ef4444":pasada?"#1e2535":S.card.border, opacity:pasada?0.8:1}}>
      ...
      {v.nota && <div style={{fontSize:13,color:"var(--text3)",fontStyle:"italic",marginBottom:8}}>"{v.nota}"</div>}
      <div style={{display:"flex",gap:6,marginTop:4,flexWrap:"wrap"}}>
        {c.id && <button style={{...S.primaryBtn,fontSize:12,padding:"7px 12px"}} onClick={()=>onVerCliente(c)}>Ver cliente</button>}
        {(c.telefono||v.clienteTelefono) && (
          <button style={{...S.waBtn,fontSize:12,padding:"7px 12px"}}
        ...
        <button style={{background:"#0a1a2a",border:"1px solid #1e40af",color:"#60a5fa",borderRadius:8,padding:"6px 10px",fontSize:12,fontWeight:600,cursor:"pointer"}}
          onClick={()=>onEditar(v)}>✏️ Editar</button>
        {c.id && <button style={{background:"#0d2010",border:"1px solid #166534",color:"#4ade80",borderRadius:8,padding:"6px 10px",fontSize:12,fontWeight:600,cursor:"pointer"}}
          onClick={()=>onEditarCliente(c)}>👤 Datos</button>}
        <button style={{...S.ghostBtn,fontSize:12,padding:"6px 10px",color:"#f87171",borderColor:"#450a0a"}} onClick={()=>onEliminar(v.id)}>✕</button>
      </div>
    </div>
  );
}

function AgendaView({ agenda, clientes, onSave, onBack, onVerCliente, showToast, onUpdateCliente }) {
  ...
  const abrirEditarVisita = (v) => {
    ...
    setEditForm({fecha:v.fecha, hora:v.hora, nota:v.nota||"", motivo:v.motivo||""});
    setModo("editarVisita");
  };

  const guardarEdicionVisita = () => {
    const nuevas = agenda.map(v=>v.id===editingVisitaId ? {...v, fecha:editForm.fecha, hora:editForm.hora, nota:editForm.nota, motivo:editForm.motivo} : v);
    onSave(nuevas);
      ...
      {modo==="nueva" && (
        <div style={S.card}>
          ...
          <div style={{marginBottom:12}}>
            ...
            <input style={S.inp} placeholder="Ej: Medición, Entrega, Instalación..." 
          ...
          <div style={{marginBottom:14}}>
            ...
            <textarea style={{...S.inp,resize:"none",height:68,lineHeight:1.5}} 
      ...
      {tab==="fab"&&(
        <div>
          ...
          {c.obsModo==="describir" && (c.aberturas||[]).length>0 ? (
            <div style={S.card}>
              <div style={S.cardTitle}>📐 Aberturas a fabricar ({(c.aberturas||[]).reduce((s,a)=>s+(parseInt(a.cantidad)||0),0)} unidades)</div>
              {(c.aberturas||[]).map(ab=>{
                const t=TIPOS.find(t=>t.id===ab.tipo);
                ...
                const medidas = esMamparaL
                  ? `Frontal ${ab.anchoFrontal||"?"}cm × Lateral ${ab.anchoLat||"?"}cm × Alto ${ab.alto||"?"}cm`
                  : ab.paredIrregular
                    ? `Sup ${ab.anchoSup||"?"}cm / Inf ${ab.anchoInf||"?"}cm × ${ab.alto||"?"}cm`
                    : `${ab.ancho||ab.anchoSup||"?"}×${ab.alto||"?"}cm`;
                return (
                  <div key={ab.id} style={{borderBottom:`1px solid #0c1525`,padding:"14px 0",display:"flex",gap:12,alignItems:"flex-start"}}>
                    ...
                    <div style={{flex:1}}>
                      ...
                      <div style={{display:"flex",flexWrap:"wrap",gap:"4px 12px",marginTop:4}}>
                        <span style={{background:"#0a2a3a",border:"1px solid #1e3a5f",borderRadius:6,padding:"3px 9px",fontSize:13,fontWeight:700,color:"#e2e8f0"}}>
                          📏 {medidas}
                        </span>
                        <span style={{background:"#1a1a2e",border:"1px solid #2d2d5e",borderRadius:6,padding:"3px 9px",fontSize:13,fontWeight:700,color:"#a78bfa"}}>
                          ×{ab.cantidad||1} unid.
                        </span>
                      </div>
                      {/* Detalles extra */}
                      <div style={{display:"flex",flexWrap:"wrap",gap:"4px 10px",marginTop:6}}>
                        ...
                        {ab.divisor&&<span style={{fontSize:12,color:"#64748b"}}>Con divisor</span>}
                        ...
                        {ab.relleno&&<span style={{fontSize:12,color:"#64748b"}}>Diseño: <b style={{color:"#94a3b8"}}>{RELLENOS.find(r=>r.id===ab.relleno)?.label||""}</b></span>}
                      </div>
                      {ab.observaciones&&<div style={{color:"#475569",fontStyle:"italic",fontSize:11,marginTop:4,borderTop:"1px solid #0c1525",paddingTop:4}}>📝 "{ab.observaciones}"</div>}
                    </div>
                  </div>
                );
              })}
