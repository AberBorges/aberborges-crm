...
export default function App() {
  ...
  const cargarDatos = async () => {
    try {
      ...
      if (e1) throw e1;
      if (e2) throw e2;
      if (cls) setClientes(cls.map(r => r.data).filter(Boolean));
      if (ag)  setAgenda(ag.map(r => r.data).filter(Boolean));
      setDbReady(true);
    } catch(e) {
      console.error('Error cargando datos:', e);
      // NO usar DEMO — dejar lista vacía y reintentar
      setDbReady(true);
    }
  };

  // Guardar UN solo cliente (más rápido y seguro)
    ...
    <div style={S.app}>
      ...
      <header style={S.header} className="no-print">
        ...
        <div style={S.hRight}>
          ...
          {view==="lista"&&(
            <button style={{...S.ghostBtn,fontSize:13,padding:"8px 12px",borderColor:"#34d39955",color:"#34d399"}}
              onClick={()=>setView("obras")}>📦 Obras</button>
          )}
          {view==="lista"&&(role==="presupuestador"||role==="admin"||role==="deudores")&&(
            <button style={{...S.ghostBtn,fontSize:13,padding:"8px 12px",borderColor:"#f43f5e55",color:"#f43f5e"}}
      ...
      {(view==="lista"&&role==="deudores")||(view==="deudores")?(
        <DeudoresView supabase={supabase} showToast={showToast}/>
      ):null}
...
// ═══════════════════════════════════════════════════════════
function DeudoresView({ supabase, showToast }) {
  ...
  const emptyDeudor = () => ({
    ...
    moneda:"UYU", // "UYU" | "USD"
    concepto:"",
    total:"", pagos:[], fechaVencimiento:"", notas:"", creadoEn:today(),
  });

  // Cargar deudores de Supabase
  useEffect(()=>{
    const cargar = async () => {
      try {
        const {data,error} = await supabase.from('deudores').select('*').order('created_at');
        if(error) throw error;
        if(data) setDeudores(data.map(r=>r.data).filter(Boolean));
  ...
  const persistir = async (nuevos) => {
    setDeudores(nuevos);
    try {
      const {error} = await supabase.from('deudores').upsert(
        nuevos.map(d=>({id:d.id, data:d})), {onConflict:'id'}
      );
      if(error) throw error;
  ...
  const guardar = async (d) => {
    const nuevos = deudores.find(x=>x.id===d.id)
      ? deudores.map(x=>x.id===d.id?d:x)
      : [...deudores, d];
    await persistir(nuevos);
    setSeleccionado(d);
    if(vista==="nuevo") setVista("detalle");
    showToast("Guardado ✓");
  };

  const eliminar = async (id) => {
    if(!window.confirm("¿Eliminar este deudor?")) return;
    const nuevos = deudores.filter(d=>d.id!==id);
    await persistir(nuevos);
    try { await supabase.from('deudores').delete().eq('id',id); } catch(e){}
    setVista("lista");
  };
  ...
  // Deudor individual — formulario + historial
  const DeudorDetalle = ({d}) => {
    const [f, setF] = useState(d);
    const [nuevoPago, setNuevoPago] = useState({monto:"",fecha:today(),nota:"",moneda:d.moneda||"UYU"});
    const fRef = useRef(f);
    ...
    const enviarEstadoCuenta = () => {
      ...
      const msg = [
        ...
        ``,
        (f.pagos||[]).length>0?`📋 *Historial de pagos:*\n${pagosHist}`:null,
        f.fechaVencimiento?`📅 Fecha de pago: ${fmtF(f.fechaVencimiento)}`:"",
        ``,
        `🇺🇾 *ABERTURAS BORGES* 🇺🇾`,
    ...
    return (
      <div style={{padding:"14px 14px 40px",maxWidth:520,margin:"0 auto"}}>
        ...
        <div style={{background:"#0d1626",border:"1px solid #162035",borderRadius:14,padding:16,marginBottom:12}}>
          <div style={{fontWeight:800,color:"#f1f5f9",fontSize:15,marginBottom:12}}>👤 Datos</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
            <div style={{gridColumn:"1/-1"}}>
              <label style={lbl}>Nombre *</label>
              <input style={inp} value={f.nombre} onChange={e=>setF(p=>({...p,nombre:e.target.value}))} placeholder="Juan Pérez"/>
            </div>
            <div>
              <label style={lbl}>Teléfono</label>
              <input style={inp} type="tel" value={f.telefono||""} onChange={e=>setF(p=>({...p,telefono:e.target.value}))} placeholder="099..."/>
            </div>
            <div>
              <label style={lbl}>Moneda</label>
              <select style={{...inp,padding:"8px 12px"}} value={f.moneda||"UYU"} onChange={e=>setF(p=>({...p,moneda:e.target.value}))}>
            ...
            <div>
              ...
              <input style={inp} value={f.direccion||""} onChange={e=>setF(p=>({...p,direccion:e.target.value}))} placeholder="Calle 123"/>
            </div>
            <div>
              <label style={lbl}>Localidad</label>
              <input style={inp} value={f.localidad||""} onChange={e=>setF(p=>({...p,localidad:e.target.value}))} placeholder="Mercedes"/>
            </div>
            <div style={{gridColumn:"1/-1"}}>
              <label style={lbl}>Concepto</label>
              <input style={inp} value={f.concepto||""} onChange={e=>setF(p=>({...p,concepto:e.target.value}))} placeholder="Aberturas, ventanas, etc."/>
            </div>
            <div>
              <label style={lbl}>Total deuda ({f.moneda||"UYU"})</label>
              <input style={inp} type="number" value={f.total||""} onChange={e=>setF(p=>({...p,total:e.target.value}))} placeholder="0"/>
            </div>
            <div>
              <label style={lbl}>📅 Fecha de cobro</label>
              <input style={{...inp,padding:"8px 12px"}} type="date" value={f.fechaVencimiento||""} onChange={e=>setF(p=>({...p,fechaVencimiento:e.target.value}))}/>
            </div>
            <div style={{gridColumn:"1/-1"}}>
              <label style={lbl}>Notas</label>
              <textarea style={{...inp,resize:"none",height:60}} value={f.notas||""} onChange={e=>setF(p=>({...p,notas:e.target.value}))} placeholder="Observaciones..."/>
            </div>
          </div>
          <button style={{background:ACCENT,color:"#090e18",border:"none",borderRadius:9,padding:"11px 0",fontWeight:800,fontSize:14,cursor:"pointer",width:"100%"}}
            onClick={()=>guardar(f)}>💾 Guardar datos</button>
        </div>
        ...
        {f.total&&(
          <div style={{background:"#060c14",border:`1px solid #00b4d833`,borderRadius:14,padding:14,marginBottom:12}}>
            ...
            <div style={{textAlign:"right",fontSize:11,color:"#475569"}}>{pct}% cobrado</div>
            {f.fechaVencimiento&&(()=>{
              const [y,m,d2]=f.fechaVencimiento.split("-").map(Number);
              const dias = Math.ceil((new Date(y,m-1,d2)-nowUY())/86400000);
              return <div style={{marginTop:8,fontSize:12,fontWeight:700,color:dias<=3?"#ef4444":dias<=7?"#f59e0b":"#94a3b8"}}>
                📅 Cobro: {fmtF(f.fechaVencimiento)} {dias===0?"¡Hoy!":dias<0?`— Vencida hace ${Math.abs(dias)}d`:`— en ${dias}d`}
              </div>;
            })()}
            <button style={{width:"100%",marginTop:10,background:"#0d2b22",color:"#25d366",border:"1px solid #25d36655",borderRadius:9,padding:"11px 0",fontWeight:700,fontSize:13,cursor:"pointer"}}
        ...
        {/* Registrar pago */}
        <div style={{background:"#0d1626",border:"1px solid #162035",borderRadius:14,padding:16,marginBottom:12}}>
          <div style={{fontWeight:800,color:"#f1f5f9",fontSize:15,marginBottom:12}}>💳 Registrar pago / entrega</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
            <div>
            ...
            <div>
              ...
              <input style={{...inp,padding:"8px 12px"}} type="date" value={nuevoPago.fecha} onChange={e=>setNuevoPago(p=>({...p,fecha:e.target.value}))}/>
            </div>
            <div style={{gridColumn:"1/-1"}}>
          ...
          <div style={{display:"flex",gap:8}}>
            ...
            <button style={{flex:1,background:"#0d2b22",color:"#25d366",border:"1px solid #25d36655",borderRadius:9,padding:"11px 0",fontWeight:700,fontSize:13,cursor:"pointer"}}
              onClick={()=>{
                const montoPago=parseFloat(nuevoPago.monto)||0;
                const nuevoSaldo=Math.max(0,(parseFloat(f.total)||0)-(totalPagado+montoPago));
                const msg=[
                  `💳 *Pago recibido — Aberturas Borges*`,`▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬`,
                  `👤 *${f.nombre}*`,`📅 ${fmtF(nuevoPago.fecha)}`,``,
                  montoPago>0?`✅ *${nuevoPago.nota||"Pago"}: ${fmtM(montoPago,f.moneda)}*`:"",
                  `💰 Total: ${fmtM(f.total,f.moneda)}`,
                  `📥 Cobrado: ${fmtM(totalPagado+montoPago,f.moneda)}`,
                  `⏳ Saldo: *${fmtM(nuevoSaldo,f.moneda)}*`,``,
        ...
        {(f.pagos||[]).length>0&&(
          <div style={{background:"#0d1626",border:"1px solid #162035",borderRadius:14,padding:16}}>
            <div style={{fontWeight:800,color:"#f1f5f9",fontSize:15,marginBottom:12}}>📋 Historial</div>
  ...
  const conSaldo = deudores.filter(d=>{
    ...
    return (parseFloat(d.total)||0)-pagado>0;
  });
  const saldados = deudores.filter(d=>{
    const pagado=(d.pagos||[]).reduce((s,p)=>s+(parseFloat(p.monto)||0),0);
    return (parseFloat(d.total)||0)-pagado<=0 && d.total;
  });

  if(vista==="detalle"&&seleccionado) return <DeudorDetalle d={seleccionado}/>;
  if(vista==="nuevo") return <DeudorDetalle d={emptyDeudor()}/>;
    ...
    <div style={{padding:"14px 14px 40px",maxWidth:520,margin:"0 auto"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
        ...
        <button style={{background:"#f43f5e",color:"#fff",border:"none",borderRadius:10,padding:"9px 16px",fontWeight:700,fontSize:13,cursor:"pointer"}}
          onClick={()=>setVista("nuevo")}>+ Nuevo</button>
      </div>
      ...
      {!cargando&&deudores.length===0&&(
        <div style={{textAlign:"center",color:"#334155",padding:"60px 20px",fontSize:14,lineHeight:1.8}}>
          ...
          No hay deudores registrados.<br/>Tocá "+ Nuevo" para agregar uno.
        </div>
      )}
      ...
      {conSaldo.length>0&&(
        <div style={{marginBottom:20}}>
          ...
          {conSaldo.sort((a,b)=>(a.fechaVencimiento||"9999")>(b.fechaVencimiento||"9999")?1:-1).map(d=>{
            ...
            const dias=d.fechaVencimiento?Math.ceil((new Date(y,m-1,day)-nowUY())/86400000):null;
            const vencida=dias!==null&&dias<0;
            return (
              <div key={d.id} style={{background:"#0d1626",border:`1px solid ${vencida?"#ef444455":"#f43f5e33"}`,borderRadius:14,padding:"14px 16px",marginBottom:8,cursor:"pointer"}}
                ...
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                  ...
                  <div style={{textAlign:"right"}}>
                    <div style={{fontWeight:900,fontSize:17,color:"#f43f5e"}}>{d.moneda==="USD"?`USD ${Number(saldo).toLocaleString("es-UY")}`:`$${Number(saldo).toLocaleString("es-UY")}`}</div>
                    <div style={{fontSize:10,color:"#475569",marginTop:2}}>saldo</div>
                  </div>
                </div>
                {d.fechaVencimiento&&(
                  <div style={{marginTop:8,fontSize:12,fontWeight:700,color:vencida?"#ef4444":dias<=3?"#f59e0b":"#94a3b8"}}>
                    📅 {fmtF(d.fechaVencimiento)} {vencida?`⚠️ Vencida hace ${Math.abs(dias)}d`:dias===0?"¡Hoy!":dias<=3?`¡En ${dias}d!`:`en ${dias}d`}
                  </div>
                )}
              </div>
            );
...
function DetailView({cliente,role,agenda,onSaveAgenda,onUpdate,onDelete,showToast}){
  ...
  useEffect(()=>{ setC(cliente); cRef.current=cliente; },[cliente]);
  ...
  const guardarObra = (extra={}) => save({
    aberturas:cRef.current.aberturas,
    notas:cRef.current.notas,
    obsModo:cRef.current.obsModo,
    fechaMedicion:cRef.current.fechaMedicion,
    ...extra
  });
  ...
  const TABS=[
    ...
    {key:"obras",  label:`📋 Obras${(c.obras||[]).length>0?" ("+c.obras.length+")":""}`, show:canM||canF},
