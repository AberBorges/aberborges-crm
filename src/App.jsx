...
const STATUS_META = {
  ...
  aprobado:      { label:"Aprobado para hacer",    color:"#34d399", icon:"✅" },
...
const SERIE_DESC = {
  "Serie 20": "con Vidrio de {mm}, Cubetas, Cierre Lara, Rodamientos de Nylon de alta durabilidad y felpas de ajuste que reducen ruidos y vibraciones por viento.",
  "Serie 25": "con Vidrio de {mm}, Cubetas, Cierre Lara, Rodamientos con Rulemán para una mejor durabilidad y deslizamiento, felpas de ajuste que reducen ruidos y vibraciones por viento.",
};
...
function buildWA(c) {
  ...
  if (c.obsModo==="describir" && (c.aberturas||[]).length>0) {
    ...
    c.aberturas.forEach((a,i)=>{
      ...
      const vt           = vidrioTexto(a.vidrio);
      const mm           = a.vidrio?.mm ? `${a.vidrio.mm}mm` : (a.vidrio?.dvh1?`DVH ${a.vidrio.dvh1}+${a.vidrio.dvh2}+${a.vidrio.dvh3}`:"");

      lineas.push(`*${i+1}) ${aberturaResumen(a)}*`);

      // Descripción técnica de serie
      if (a.serie && SERIE_DESC[a.serie]) {
        const desc = SERIE_DESC[a.serie].replace("{mm}", mm||"?mm");
        lineas.push(`   📌 ${desc}`);
      }

      // Precio
      if (subtotal>0) {
        if (precio>0) lineas.push(`   💲 Abertura: ${fmtMoney(precio)} × ${cant} = *${fmtMoney(subtotalAb)}*`);
        if (precioMono>0) lineas.push(`   📦 Monoblock: ${fmtMoney(precioMono)} × ${cant} = *${fmtMoney(subtotalMono)}*`);
        if (precio>0 && precioMono>0) lineas.push(`   ✅ Subtotal: *${fmtMoney(subtotal)}*`);
        else if (precio>0 || precioMono>0) lineas.push(`   ✅ Subtotal: *${fmtMoney(subtotal)}*`);
      }
      lineas.push("");
    });
  } else if (c.notas) {
  ...
  if (pagado>0) lineas.push(`✔️ Pagado: ${fmtMoney(pagado)}`);
  if (saldo>0)  lineas.push(`⏳ Saldo pendiente: ${fmtMoney(saldo)}`);
  else if (pagado>0) lineas.push(`🎉 ¡Totalmente saldado!`);
  lineas.push("");
  lineas.push(`🙏 _Gracias por elegirnos. Quedamos a su disposición._`);
  lineas.push(`🏠 _Aberturas Borges_`);
    ...
    <div style={S.page}>
      ...
      {tab==="presup"&&canF&&(
        <div>
          ...
          {(c.status==="presupuestado"||c.status==="aprobado"||c.status==="cancelado")&&(
            <div style={{...S.card}}>
