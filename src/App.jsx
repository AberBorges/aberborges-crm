      ...
      <div style={{background:"#0d1626",border:`1px solid ${pct===100?"#34d39966":ACCENT+"44"}`,borderRadius:14,padding:"14px 16px",marginBottom:10}}>
        ...
        {expandida&&(
          <div style={{marginTop:14}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:8,marginTop:12}}>
              {pendientes.length>0&&(
                <button style={{background:"#25d366",color:"#000",border:"none",borderRadius:10,padding:"13px 0",fontWeight:800,fontSize:14,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8,
                  opacity:Object.values(seleccionadas).some(v=>v)?1:0.4}}
                  onClick={marcarEntregadas}>
                  💬 Registrar entrega y avisar por WhatsApp
                </button>
              )}
              <button style={{background:"transparent",border:`1px solid ${ACCENT}44`,color:ACCENT,borderRadius:10,padding:"9px 0",fontSize:13,cursor:"pointer"}}
                onClick={()=>onSelect(c)}>Ver ficha completa →</button>
            </div>
          </div>
        )}
      </div>
